...
static inline void _nocheck__trace_clock_set(const char * clk, uint64_t old, uint64_t new)
{
    if (trace_event_get_state(TRACE_CLOCK_SET) && qemu_loglevel_mask(LOG_TRACE)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log("%d@%zu.%06zu:clock_set " "'%s', ns=%"PRIu64"->%"PRIu64 "\n",
                 qemu_get_thread_id(),
                 (size_t)_now.tv_sec, (size_t)_now.tv_usec
                 , clk, old, new);
    }
...
static inline void _nocheck__trace_clock_update(const char * clk, const char * src, uint64_t val, int cb)
{
    if (trace_event_get_state(TRACE_CLOCK_UPDATE) && qemu_loglevel_mask(LOG_TRACE)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log("%d@%zu.%06zu:clock_update " "'%s', src='%s', ns=%"PRIu64", cb=%d" "\n",
                 qemu_get_thread_id(),
                 (size_t)_now.tv_sec, (size_t)_now.tv_usec
                 , clk, src, val, cb);
    }
...
static inline void trace_clock_update(const char * clk, const char * src, uint64_t val, int cb)
{
    if (true) {
        _nocheck__trace_clock_update(clk, src, val, cb);
    }
}
