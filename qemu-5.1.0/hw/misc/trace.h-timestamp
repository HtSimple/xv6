...
extern TraceEvent _TRACE_EMPTY_SLOT_READ_EVENT;
...
extern uint16_t _TRACE_EMPTY_SLOT_READ_DSTATE;
...
#define TRACE_EMPTY_SLOT_READ_ENABLED 1
...
#define TRACE_EMPTY_SLOT_READ_BACKEND_DSTATE() ( \
    trace_event_get_state_dynamic_by_id(TRACE_EMPTY_SLOT_READ) || \
    false)

static inline void _nocheck__trace_empty_slot_read(uint64_t addr, unsigned width, uint64_t value, unsigned size, const char * name)
{
    if (trace_event_get_state(TRACE_EMPTY_SLOT_READ) && qemu_loglevel_mask(LOG_TRACE)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log("%d@%zu.%06zu:empty_slot_read " "rd addr:0x%04"PRIx64" data:0x%0*"PRIx64" size %u [%s]" "\n",
                 qemu_get_thread_id(),
                 (size_t)_now.tv_sec, (size_t)_now.tv_usec
...
static inline void trace_empty_slot_read(uint64_t addr, unsigned width, uint64_t value, unsigned size, const char * name)
{
    if (true) {
        _nocheck__trace_empty_slot_read(addr, width, value, size, name);
    }
}
