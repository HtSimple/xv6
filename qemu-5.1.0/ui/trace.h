...
static inline void _nocheck__trace_ppm_save(int fd, void * display_surface)
{
    if (trace_event_get_state(TRACE_PPM_SAVE) && qemu_loglevel_mask(LOG_TRACE)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log("%d@%zu.%06zu:ppm_save " "fd=%d surface=%p" "\n",
                 qemu_get_thread_id(),
                 (size_t)_now.tv_sec, (size_t)_now.tv_usec
                 , fd, display_surface);
    }
...
static inline void trace_ppm_save(int fd, void * display_surface)
{
    if (true) {
        _nocheck__trace_ppm_save(fd, display_surface);
    }
}
