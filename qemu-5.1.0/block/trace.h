...
static inline void _nocheck__trace_nvme_process_completion_queue_plugged(void * s, int index)
{
    if (trace_event_get_state(TRACE_NVME_PROCESS_COMPLETION_QUEUE_PLUGGED) && qemu_loglevel_mask(LOG_TRACE)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log("%d@%zu.%06zu:nvme_process_completion_queue_plugged " "s %p queue %d" "\n",
                 qemu_get_thread_id(),
                 (size_t)_now.tv_sec, (size_t)_now.tv_usec
                 , s, index);
    }
...
static inline void trace_nvme_process_completion_queue_plugged(void * s, int index)
{
    if (true) {
        _nocheck__trace_nvme_process_completion_queue_plugged(s, index);
    }
}
